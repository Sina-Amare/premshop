...
| C12 | Login OTP — auth flow (D6) | email, **or telegram if linked** | «کد ورود شما به پرم‌شاپ: {otp_code} — تا ۱۰ دقیقه معتبر است. اگر شما درخواست نداده‌اید، این پیام را نادیده بگیرید.» — **bypasses the outbox** (direct Celery task, single attempt + one immediate retry; a backed‑off OTP is a dead OTP) |
