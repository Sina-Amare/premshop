...
Read `docs-local/progress.md` first, every session — its "Where we are now" block is the current state, rewritten each step; below it is a newest-first log per step (working agreement §6), never rewritten — then `docs-local/learning.md`. Open `learning-reference.md` (concept map, answer keys) only to write a brief or grade a check-back. Continuity lives in files, not in memory.
...
- **`progress.md` is both** the template's short current-state file (§29) — the "Where we are now" block at the top — and the working agreement's newest-first log per step (§6) below it.
...
- **The contracts** — `docs/data-model.md`, `docs/state-machine.md`, `docs/services-and-modules.md` — exist so the same decision isn't remade three ways across sessions. Each hardens at the step that builds it. If mid-build you discover a contract is *wrong*, **stop and raise it**; changing a contract is a conversation. Changing a filename isn't. The build plan (`docs-local/build-plan.md`) points to these documents instead of restating them; a line marked **⚠ Disagrees with …** in a step's section is a place where the two said different things, and **that step's briefing settles every ⚠ in its section with the owner** before building — then the flag is deleted and the losing document corrected. The design documents still restate each other (event names appear in four of them; the data model's field names are restated in the other three); preparing a step's briefing includes bringing **that step's** sections into line with the ownership table in `docs/README.md` — restatements become references, disagreements join the briefing. Not all at once: sections for unbuilt later steps will be revised when their step arrives anyway.
...
The owner does not write the code, but must understand, manage and defend all of it — that is the difference between owning this project and vibe-coding it. This agent builds, and is **also responsible for running the loop below so no part of it is quietly skipped** — and for keeping it light: a ritual that adds nothing gets dropped, and trivial changes get none of it. The ledger is `docs-local/learning.md` — study briefs, check-back questions, the log, the parking lot, the current independence stage — with the concept map and answer keys in `learning-reference.md`.
...
4. **Check-back before the next go.** The owner explains one or two load-bearing pieces in their own words, graded against the code. Answer keys in `learning-reference.md` name the file and the function or constraint; line numbers drift, so a key is refreshed just before it is used. Plus **one owner-run check against a real system** — the Actions run, an email's "Show original", a database row. Someone other than the agent has to look.
...
**Outside the repo — `docs-local/`, gitignored:** `progress.md`, `open-questions.md`, `learning.md` and `learning-reference.md`, the build plan (*when* each thing is built — it points to the design documents for *what*, and never restates them), the original product brief (cost prices, margins, supplier arrangements) and working agreement, and in `archive/` the finished design reviews, question rounds and step plans. Its `README.md` maps which file answers which question. It has its own local git history (`git -C docs-local log`), separate from the public repo, and no backup yet (open question #14). These files are internal or commercially sensitive; conclusions from them live in ADRs, which is what the repo carries.
